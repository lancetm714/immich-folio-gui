...
export const VALID_HERO_STYLES = ['split', 'fullbleed', 'minimal', 'stacked', 'typographic', 'mosaic'];
...
export function resolveTheme(raw?: SettingsYaml['theme']): ThemeConfig {
    ...
    if (typeof raw === 'string') {
        ...
        if (!preset) {
            throw new Error(`Unknown theme preset "${raw}". Valid presets: ${Object.keys(THEME_PRESETS).join(', ')}`);
        }
        return { ...preset };
    }
    ...
    if (!base) {
        throw new Error(`Unknown theme preset "${baseName}". Valid presets: ${Object.keys(THEME_PRESETS).join(', ')}`);
    }
